...
ok($("#fiche-chantier h2").textContent === "Résumé" && $$("#fiche .resume-figs figure svg").length === 1 && $$("#fiche ul.legende li").length >= 5 && /palissade|grillage/.test($("#fiche ul.legende").textContent) && $$("#fiche ul.resume-points li").length === 4 && $("#fiche .resume-figs").nextElementSibling === $("#fiche ul.resume-points") && /TTC/.test($("#fiche").textContent), "résumé : paragraphe, dessin avec sa légende à côté, puis quatre puces de chiffres");
...
ok(["voir-toit", "voir-murs", "voir-porte", "voir-mobilier", "voir-lit", "voir-etiquettes", "voir-personne", "voir-cloture"].every((id) => $("#" + id) && $("#" + id).tagName === "BUTTON" && $("#" + id + " svg") && $("#" + id + " span") && $$("#" + id + " .points b").length === +$("#" + id).dataset.etats) && $("#voir-porte span").textContent === "porte" && $("#voir-porte").getAttribute("aria-pressed") === "true" && $("#voir-cloture").dataset.etat === "0" && $("#voir-cloture span").textContent === "clôture" && $$("#bascules .groupe").length === 3 && $("#bascules").parentElement.classList.contains("scene") && $("#bascules").previousElementSibling.classList.contains("vue-barre") && $$("#bascules .groupe")[1].querySelectorAll("button span").length === 3 && [...$$("#bascules .groupe")[1].querySelectorAll("button span")].map((x) => x.textContent).join(" ") === "porte lit personne", "bascules : colonne posée sur la vue (dans .scene, après la barre caméra), libellés fixes, autant de points que d'états, trois groupes (toit murs | porte lit personne | mobilier repères clôture)");
