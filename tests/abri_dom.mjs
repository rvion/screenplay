...
ok(/th, td \{[^}]*white-space: nowrap/.test(readFileSync(join(ROOT, "site/abri.css"), "utf8")) && /\.bom td \.regle \{ white-space: normal/.test(readFileSync(join(ROOT, "site/abri.css"), "utf8")) && $$("main table").every((t) => t.parentElement.classList.contains("table-wrap")) && $$("#murs tbody tr").every((tr) => !tr.children[0].classList.contains("pliable") && tr.children[3].classList.contains("pliable") && tr.children[4].classList.contains("pliable")) && $$("#murs tbody td:first-child .paire").length === m.faces.length && $$("#murs .paire .face").length >= m.faces.length * 2 && $$("#fiche .carte:first-child .paire").length === m.faces.length, "tableaux : sans repli sauf les colonnes pliables (hauteur finie, panneaux, textes) ; le nom du mur est insécable");
...
ok($$("a.zoom[data-zoom]").length === $$("main .planbox").length && $$("a.zoom[data-zoom]").every((z) => a.core.planches[z.dataset.zoom]) && $("#plan-implantation a.zoom") && $("#plan-sol a.zoom"), "chaque planche de la page a son lien « agrandir » (nouvel onglet)");
...
ok($$("#alternatives-corps .alt").length === 4 && $$("#alternatives-corps .alt svg").length === 4 && $$("#alternatives-corps .alt b").map((b) => b.textContent).join("|") === "Everbox 2 × 2 m|Carré 2 × 2 m|Rectangle 200 × 240|Trapèze" && $$("#alternatives-corps a.alt").every((x) => existsSync(join(ROOT, "site", x.getAttribute("href").split("#")[0]))) && !/Option 1|Rectangle 200 × 240 ·/.test($("#alternatives-corps svg").innerHTML), "formes étudiées : Everbox du commerce, carré 2 × 2, rectangle 200 × 240, trapèze ; dessins sans entête ; liens existants");
