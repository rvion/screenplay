...
ok($("#fiche-chantier h2").textContent === "Résumé" && $$("#fiche .resume-figs figure svg").length === 2 && $$("#fiche dl dt").length === 4 && /TTC/.test($("#fiche").textContent), "résumé : deux dessins (murs et angles, marges sur la dalle) et quatre lignes de chiffres");
ok(m.faces.every((f) => new RegExp(f.cle + " " + String(f.longueur_cm).replace(".", ",")).test($("#fiche figure:first-child svg").textContent)) && /135°/.test($("#fiche figure:first-child svg").textContent), "résumé : le dessin des murs porte chaque longueur (A 200 … G 275) et les angles");
ok(/\b10\b/.test($("#fiche figure:last-child svg").textContent) && /57,8/.test($("#fiche figure:last-child svg").textContent) && /52/.test($("#fiche figure:last-child svg").textContent) && /grillage/.test($("#fiche figure:last-child").textContent), "résumé : le dessin des marges porte 10 (gauche, devant), 52 (droite) et le passage 57,8, avec le grillage");
