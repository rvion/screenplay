...
ok($("#fiche-chantier h2").textContent === "Résumé" && $$("#fiche .resume-figs figure svg").length === 1 && $$("#fiche ul.legende li").length >= 6 && /palissade|grillage/.test($("#fiche ul.legende").textContent) && $$("#fiche ul.resume-points li").length === 4 && $("#fiche .resume-figs").nextElementSibling === $("#fiche ul.resume-points") && /TTC/.test($("#fiche").textContent), "résumé : paragraphe, dessin avec sa légende à côté, puis quatre puces de chiffres");
ok(m.faces.every((f) => new RegExp(f.cle + " " + String(f.longueur_cm).replace(".", ",")).test($("#fiche figure svg").textContent)) && /135°/.test($("#fiche figure svg").textContent) && /\b10\b/.test($("#fiche figure svg").textContent) && /57,8/.test($("#fiche figure svg").textContent) && /52/.test($("#fiche figure svg").textContent) && !$("#fiche figcaption"), "résumé : le dessin porte chaque mur (A 200 … G 275), les angles, les marges 10, 10, 52 et le passage 57,8");
ok(/cinq murs/.test($("#intro").textContent) && !/à 10/.test($("#intro").textContent) && $$("#intro .face").length >= 3, "résumé : introduction courte (murs, toit, porte, fenêtres, bureaux), sans la marge dans la phrase");
...
ok($$("#fiche .cote").length >= 6 && $$("#fiche .face").length >= 2 && $$("#murs .face").length === m.faces.length + m.faces.reduce((n, f) => n + f.panneaux.length, 0) && $$("#debit-murs .face, #debit-toit .face, #debit-rehausse .face").length >= 12, "cotes (.cote) et repères (.face) balisés dans la fiche, les murs et le débit");
...
ok($$("#pourquoi-corps .pourquoi-bloc").length === 3 && $$("#pourquoi-corps li b").length >= 10 && $$("#pourquoi-corps li .suite").length >= 10 && $$("#pourquoi-corps h3 .marque").length === 3 && !/version 3|au lieu de/.test($("#pourquoi-corps").textContent), "pourquoi : points forts, points faibles, questions ; textes autonomes (aucune comparaison avec une autre version)");
ok($$("#pourquoi-corps .questions li .question").map((q) => q.textContent).join() === "Q1,Q2,Q3,Q4,Q5,Q6" && $$("#pourquoi-corps .questions li").every((li) => /\?$/.test(li.textContent.trim())) && /8,2 %/.test($("#pourquoi-corps .questions").textContent), "questions ouvertes : Q1 à Q6, chacune finit par un point d'interrogation, champs calculés remplacés");
...
ok(["voir-toit", "voir-porte", "voir-mobilier", "voir-lit", "voir-etiquettes", "voir-personne", "voir-cloture"].every((id) => $("#" + id) && $("#" + id).tagName === "BUTTON" && $("#" + id + " svg") && $("#" + id + " span")) && $("#voir-personne").dataset.etats === "3" && $$("#voir-personne .points b").length === 3 && $("#voir-porte").dataset.etats === "3" && $("#voir-porte span").textContent === "ouverte" && $("#voir-porte").getAttribute("aria-pressed") === "true" && $("#voir-personne").getAttribute("aria-pressed") === "false", "bascules de la 3D : boutons carrés à icône (porte active), personne à trois états avec ses trois points, éteinte au départ");
...
ok($$("#vignettes button[data-vue] canvas").length === 6 && $$("#vignettes button").map((b) => b.dataset.vue).join() === "jardin,porte,arriere,droite,interieur,lit" && $("#vignettes").previousElementSibling.classList.contains("fiche-vue"), "six vignettes sous les deux colonnes du résumé, la première est la vue de départ");
