...
// ce que la page lit vraiment : params.js (params.json + les plans figes des formes archivees)
const params = JSON.parse(readFileSync(join(ROOT, "site/params.js"), "utf8").replace(/^[\s\S]*?window\.SHED_PARAMS = /, "").replace(/;\s*$/, ""));
...
ok($("#fiche-chantier h2").textContent === "Résumé" && $$("#fiche .resume-figs figure svg").length === 1 && $$("#fiche ul.legende li").length >= 5 && /palissade|grillage/.test($("#fiche ul.legende").textContent) && !$("#tags") && $$("#intro .fait").length >= 8 && /TTC/.test($("#intro").textContent) && $("#intro").textContent.includes(String(m.pente.pourcent).replace(".", ",")) && /·/.test($("#intro").textContent) && $("#intro").nextElementSibling === $("#fiche"), "résumé : un paragraphe de faits séparés par des points médians, faits clés surlignés, puis le dessin avec sa légende");
ok(m.faces.every((f) => $$("#fiche figure svg text").some((t) => t.textContent === f.cle) && $$("#fiche figure svg text").some((t) => t.textContent === String(f.longueur_cm).replace(".", ","))) && $$("#fiche figure svg path").length === m.faces.length && m.angles_deg.filter((a) => a !== 90).every((a) => $("#fiche figure svg").textContent.includes(String(a).replace(".", ",") + "°")) && /\b10\b/.test($("#fiche figure svg").textContent) && $("#fiche figure svg").textContent.includes(String(v.passages.find((pa) => pa.cote === "arriere_droite").cm).replace(".", ",")) && !$("#fiche figcaption"), "résumé : le dessin porte chaque mur, les angles, les marges et le passage derrière (" + v.passages.find((pa) => pa.cote === "arriere_droite").cm + ")");
...
// materiaux : que des achats, quantite x prix = montant, total = somme hors options, rien de forfaitaire
{
  ...
  ok($$("#materiaux .table-wrap").every((w) => w.classList.contains("fixe")) && $$("#materiaux table.bom thead th").slice(0, 4).map((t) => t.textContent).join("|") === "matériau|qté|p.u.|montant" && /\.table-wrap\.fixe \{ overflow: visible/.test(readFileSync(join(ROOT, "site/abri.css"), "utf8")) && /\.bom td \.poste \{ white-space: normal; font-weight: 600/.test(readFileSync(join(ROOT, "site/abri.css"), "utf8")) && /\.bom th\.num, \.bom td\.num \{ width: 1%/.test(readFileSync(join(ROOT, "site/abri.css"), "utf8")), "matériaux : le tableau ne défile jamais, entêtes courtes, colonnes de chiffres serrées, nom de l'article en demi-gras qui se replie");
...
// guide de montage : meme composant, liste des etapes a gauche, l'etape choisie a droite
{
  ...
  ok(/à plat/.test($("#etapes").textContent) && !/\bpanne\b/i.test($("#etapes").textContent) && !$$("main td").some((td) => /^Panne\b/.test(td.textContent.trim())) && /panne en travers/.test($("#questions-corps").textContent) && !/undefined|NaN/.test($("#etapes").textContent), "guide : mur gauche monté à plat ; la panne, ni dessinée ni achetée ni montée, reste une question ; aucune valeur manquante");
...
ok($$("#ouvertures-table tbody tr").length === 1 + v.fenetres.length && /porte pleine/.test($("#ouvertures-table").textContent) && $$("#mobilier #amenagement tbody tr").length >= 3 && $("#ouvertures").parentElement === $("main") && $("#ouvertures").nextElementSibling === $("#mobilier"), "ouvertures et mobilier : deux boîtes l'une sous l'autre");
ok($$("#debit-murs td .cote .n").length >= 10 && $$("main td .cote").some((c) => c.querySelector(".d")) && $$("main td .cote .d").every((d) => /^,\d+$/.test(d.textContent)), "cotes des tableaux : partie entière et décimales séparées (alignement sur le dernier chiffre entier)");
...
ok($("#questions h2").textContent === "Questions et idées" && $("#pourquoi").nextElementSibling === $("#questions") && $$("#questions-corps .pourquoi-bloc").length === 2 && $$("#questions-corps ol.questions").every((o) => o.closest(".pourquoi-bloc")) && $$("#questions-corps .pourquoi-bloc:first-child .question").map((q) => q.textContent).every((q, i) => q === "Q" + (i + 1)) && $$("#questions-corps .pourquoi-bloc:first-child li").every((li) => /\?$/.test(li.textContent.trim())) && $("#questions-corps").textContent.includes(String(m.pente.pourcent).replace(".", ",") + " %"), "questions : leur section sous « pourquoi », un bloc de questions numérotées en liste simple, chacune finit par « ? »");
ok($$("#questions-corps .pourquoi-bloc:last-child .question").length === params.abri.dossier.idees.length && $$("#questions-corps .pourquoi-bloc:last-child .question")[0].textContent === "I1" && $$("#questions-corps .pourquoi-bloc:last-child li b").length >= params.abri.dossier.idees.length && (() => { const t = $("#questions-corps .pourquoi-bloc:last-child").textContent.match(/porte (\d+,\d+) m sans panne/); return !!t && Math.abs(+t[1].replace(",", ".") - m.portee_cm / 100) <= 0.05; })() && !/columns: 2/.test(readFileSync(join(ROOT, "site/abri.css"), "utf8").split("ol.questions")[1].split("}")[0]), "idées : un second bloc I1.., accroche en gras, chiffres injectés, liste simple sans colonnes de texte");
...
// document complet (?doc=1) : le meme rendu, tout deroule sans menu ni listes, c'est aussi ce que l'impression montre
{
  ...
  ok(/beforeprint/.test(readFileSync(join(ROOT, "site/src/abri_main.ts"), "utf8")) && /afterprint/.test(readFileSync(join(ROOT, "site/src/abri_main.ts"), "utf8")), "imprimer passe par le mode document, puis la page retrouve le sien");
}
ok(["voir-toit", "voir-murs", "voir-porte", "voir-mobilier", "voir-etiquettes", "voir-personne", "voir-cloture"].every((id) => $("#" + id) && $("#" + id).tagName === "BUTTON" && $("#" + id + " svg") && $("#" + id + " span") && $$("#" + id + " .points b").length === +$("#" + id).dataset.etats) && $("#voir-porte span").textContent === "porte" && $("#voir-porte").getAttribute("aria-pressed") === "true" && $("#voir-cloture").dataset.etat === "0" && $("#voir-cloture").dataset.etats === "3" && $$("#voir-cloture .points b").length === 3 && $("#voir-cloture span").textContent === "clôture" && $$("#bascules .groupe").length === 0 && $$("#bascules > button").length === 7 && $("#voir-mobilier").dataset.etats === "3" && $$("#voir-mobilier .points b").length === 3 && !$("#voir-lit") && $("#bascules").parentElement.classList.contains("scene") && $("#bascules").previousElementSibling.classList.contains("vue-barre"), "bascules : colonne posée sur la vue (dans .scene, après la barre caméra), huit boutons sans groupe, libellés fixes, autant de points que d'états");
...
ok($$("#vignettes button[data-vue] canvas").length === 8 && $$("#vignettes button").map((b) => b.dataset.vue).join() === "jardin,droite,arriere,porte,interieur,debout,dedans,couche" && $("#vignettes").previousElementSibling.classList.contains("fiche-vue"), "huit vignettes sous les deux colonnes du résumé, la première est la vue de départ");
