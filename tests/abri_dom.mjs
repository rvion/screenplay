...
// menu de gauche : une seule version montree (abri_menu), donc pas de bloc « versions » ni de lien vers une autre
{
  ...
  ok(ancres.length === 11 && ancres.every((h) => $(h)), "menu : les 11 sections de la page, toutes existantes");
...
ok(["voir-toit", "voir-porte", "voir-mobilier", "voir-lit", "voir-etiquettes", "voir-personne", "voir-cloture"].every((id) => $("#" + id) && $("#" + id).tagName === "BUTTON" && $("#" + id + " svg") && $("#" + id + " span")) && $("#voir-personne").dataset.etats === "3" && $$("#voir-personne .points b").length === 3 && $("#voir-porte").dataset.etats === "3" && $("#voir-porte span").textContent === "ouverte" && $("#voir-porte").getAttribute("aria-pressed") === "true" && $("#voir-cloture").dataset.etat === "0" && $("#voir-cloture span").textContent === "translucide" && $("#voir-personne").getAttribute("aria-pressed") === "false", "bascules de la 3D : boutons carrés à icône (porte active), personne à trois états avec ses trois points, éteinte au départ");
