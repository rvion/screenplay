...
ok(m.faces.every((f) => $$("#fiche figure svg text").some((t) => t.textContent === f.cle) && $$("#fiche figure svg text").some((t) => t.textContent === String(f.longueur_cm).replace(".", ","))) && $$("#fiche figure svg path").length === m.faces.length && /135°/.test($("#fiche figure svg").textContent) && /\b10\b/.test($("#fiche figure svg").textContent) && $("#fiche figure svg").textContent.includes(String(v.passages.find((pa) => pa.cote === "arriere_droite").cm).replace(".", ",")) && !$("#fiche figcaption"), "résumé : le dessin porte chaque mur, les angles, les marges et le passage derrière (" + v.passages.find((pa) => pa.cote === "arriere_droite").cm + ")");
...
ok($$("#vignettes button[data-vue] canvas").length === 8 && $$("#vignettes button").map((b) => b.dataset.vue).join() === "jardin,droite,arriere,porte,interieur,debout,regarder,couche" && $("#vignettes").previousElementSibling.classList.contains("fiche-vue"), "huit vignettes sous les deux colonnes du résumé, la première est la vue de départ");
