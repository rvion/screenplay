...
// guide de montage : meme composant, liste des etapes a gauche, l'etape choisie a droite
{
  ...
  ok(/à plat/.test($("#etapes").textContent) && !/\bpanne\b/i.test($("#etapes").textContent) && !$$("main td").some((td) => /^Panne\b/.test(td.textContent.trim())) && !/undefined|NaN/.test($("#etapes").textContent), "guide : mur gauche monté à plat ; la panne, ni dessinée, ni achetée, ni montée ; aucune valeur manquante");
...
ok($("#questions h2").textContent === "Questions et idées" && $("#pourquoi").nextElementSibling === $("#questions") && $$("#questions-corps .pourquoi-bloc").length === 2 && $$("#questions-corps ol.questions").every((o) => o.closest(".pourquoi-bloc")) && $$("#questions-corps .pourquoi-bloc:first-child .question").map((q) => q.textContent).every((q, i) => q === "Q" + (i + 1)), "questions : leur section sous « pourquoi », un bloc de questions numérotées en liste simple");
ok($$("#questions-corps .pourquoi-bloc:last-child .question").map((q) => q.textContent).every((q, i) => q === "I" + (i + 1)) && !/columns: 2/.test(readFileSync(join(ROOT, "site/abri.css"), "utf8").split("ol.questions")[1].split("}")[0]), "idées : un second bloc numéroté I1.., liste simple sans colonnes de texte");
