...
ok($("#fiche-chantier h2").textContent === "Résumé" && $$("#fiche .resume-figs figure svg").length === 1 && $$("#fiche ul.legende li").length >= 5 && /palissade|grillage/.test($("#fiche ul.legende").textContent) && $$("#fiche ul.resume-points li").length === 4 && $$("#fiche ul.resume-points li svg").length === 4 && $$("#fiche ul.resume-points .k").every((k) => k.textContent === k.textContent.toLowerCase()) && $("#fiche .resume-figs").nextElementSibling === $("#fiche ul.resume-points") && /TTC/.test($("#fiche").textContent), "résumé : paragraphe, dessin avec sa légende à côté, puis quatre puces de chiffres");
