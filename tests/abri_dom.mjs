...
ok(["voir-toit", "voir-murs", "voir-porte", "voir-mobilier", "voir-lit", "voir-etiquettes", "voir-personne", "voir-cloture"].every((id) => $("#" + id) && $("#" + id).tagName === "BUTTON" && $("#" + id + " svg") && $("#" + id + " span") && $$("#" + id + " .points b").length === +$("#" + id).dataset.etats) && $("#voir-porte span").textContent === "porte" && $("#voir-porte").getAttribute("aria-pressed") === "true" && $("#voir-cloture").dataset.etat === "0" && $("#voir-cloture span").textContent === "clôture" && $$("#bascules .groupe").length === 3 && $("#bascules").nextElementSibling.classList.contains("scene") && $("#bascules").nextElementSibling.firstElementChild.id === "viewer" && $$("#bascules .groupe")[1].querySelectorAll("button span").length === 3 && [...$$("#bascules .groupe")[1].querySelectorAll("button span")].map((x) => x.textContent).join(" ") === "porte lit personne", "bascules : une ligne au-dessus de la vue, libellés fixes, autant de points que d'états, trois groupes (toit murs | porte lit personne | mobilier repères clôture)");
