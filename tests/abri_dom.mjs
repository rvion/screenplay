...
ok($$("#questions-corps .pourquoi-bloc:last-child .question").length >= 9 && $$("#questions-corps .pourquoi-bloc:last-child .question")[0].textContent === "I1" && $$("#questions-corps .pourquoi-bloc:last-child li b").length >= 9 && (() => { const t = $("#questions-corps .pourquoi-bloc:last-child").textContent.match(/porte (\d+,\d+) m sans panne/); return !!t && Math.abs(+t[1].replace(",", ".") - m.portee_cm / 100) <= 0.05; })() && !/columns: 2/.test(readFileSync(join(ROOT, "site/abri.css"), "utf8").split("ol.questions")[1].split("}")[0]), "idées : un second bloc I1.., accroche en gras, chiffres injectés, liste simple sans colonnes de texte");
