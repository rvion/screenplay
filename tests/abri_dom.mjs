...
ok($("#fiche-chantier h2").textContent === "Résumé" && $$("#fiche .resume-figs figure svg").length === 1 && $$("#fiche ul.legende li").length >= 5 && /palissade|grillage/.test($("#fiche ul.legende").textContent) && $$("#tags li").length === 5 && $$("#tags li svg").length === 5 && $("#intro").nextElementSibling === $("#tags") && /TTC/.test($("#tags").textContent) && /8,2/.test($("#tags").textContent), "résumé : paragraphe, cinq étiquettes de chiffres, puis le dessin avec sa légende");
...
ok($$("#tags .cote").length >= 6 && $$("#intro .face").length >= 2 && $$("#murs .face").length === m.faces.length + m.faces.reduce((n, f) => n + f.panneaux.length, 0) && $$("#debit-murs .face, #debit-toit .face, #debit-rehausse .face").length >= 12, "cotes (.cote) et repères (.face) balisés dans la fiche, les murs et le débit");
