...
// abri actuel : l'etude v3 avec le toit vers le fond, gouttiere derriere (nervures dans le sens de la pente)
{
  ...
  ok(v4.cotes_interieures_cm[0] === 196 && JSON.stringify(m4.faces.map((f) => f.longueur_cm)) === JSON.stringify([208, 180, 99, 138, 250]) && v4.aire_interieure_m2 === 4.44 && JSON.stringify(m4.angles_deg) === JSON.stringify([90, 90, 135, 135, 90]) && m4.formalites.formalite === "aucune" && near(m4.formalites.emprise_au_sol_m2, 4.96, 0.006) && m4.faces[2].panneaux.length === 1, "abri : facade 208 (196 dedans, un lit de 190 tient), droite 180, gauche 250, pan a 45 deg de 99 cm (UN panneau), fond 138 pour le fermer, emprise 4,955 m2 donc aucune formalite");
