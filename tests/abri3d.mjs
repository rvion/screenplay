...
ok(Object.keys(VUES).join() === "jardin,droite,arriere,porte,interieur,couche" && VUES.jardin.position[2] > 3 && VUES.porte.etats.murs === 2 && VUES.porte.etats.toit === 0 && VUES.arriere.position[2] < -3 && VUES.droite.position[0] < -2 && Object.values(VUES).every((v) => v.titre.length > 5 && v.etats && Object.keys(v.etats).length === 7), "six points de vue fixes, chacun avec ses sept états d'options");
ok(["porte", "interieur", "couche"].every((k) => JSON.stringify([VUES[k].position, VUES[k].cible, VUES[k].fov]) === JSON.stringify([VUES.interieur.position, VUES.interieur.cible, VUES.interieur.fov])) && VUES.interieur.position[1] > 4, "les trois vues de l'intérieur partagent la même caméra, vue de haut");
...
const paroi = groupes.murs.children;
{
  ...
  if (d.mobilier.lit2) {
    ...
    ok(nom !== "abri actuel" || lm.map((x) => x.nom).join() === "v3", "lit à demeure : v3 (" + lm.map((x) => x.nom).join() + ")");
    lm.forEach((l, i) => {
