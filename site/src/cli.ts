...
// Cache-bust : reecrit ?v=<hash> sur abri.css / three.js / abri.js / params.js dans index.html,
// chacun le hash de son propre contenu (three.js ne change qu'avec la version de three)
function stamp() {
  const chemin = join(SITE, "index.html");
  const html = readFileSync(chemin, "utf8").replace(/(href|src)="(abri\.css|three\.js|abri\.js|params\.js)(?:\?v=[^"]*)?"/g, (_, attr, f) => {
    let v = "0";
    try { v = createHash("sha256").update(readFileSync(join(SITE, f))).digest("hex").slice(0, 8); } catch { /* bundle absent avant build */ }
    return `${attr}="${f}?v=${v}"`;
  });
