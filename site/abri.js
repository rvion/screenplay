(() => {
  ...
  var require_three = __commonJS({
    "three-global:three"(exports, module) {
      module.exports = window.ABRI_THREE["three"];
    }
  });

  // three-global:three/addons/controls/OrbitControls.js
  var require_OrbitControls = __commonJS({
    "three-global:three/addons/controls/OrbitControls.js"(exports, module) {
      module.exports = window.ABRI_THREE["three/addons/controls/OrbitControls.js"];
    }
  });

  // three-global:three/addons/environments/RoomEnvironment.js
  var require_RoomEnvironment = __commonJS({
    "three-global:three/addons/environments/RoomEnvironment.js"(exports, module) {
      module.exports = window.ABRI_THREE["three/addons/environments/RoomEnvironment.js"];
    }
  });
  ...
  function nomenclature_abri(p, v, m) {
    ...
    const refs = Object.entries(par_largeur).sort((a, b) => +a[0] - +b[0]);
    const murs_de = (l) => m.faces.filter((f) => Math.abs((f.module_cm || mod * 100) - l) < 0.05).map((f) => f.cle);
    const ou = (l) => {
      const k = murs_de(l);
      return refs.length === 1 ? "tous les murs" : k.length === 1 && k[0] === "A" ? "fa\xE7ade A" : `murs ${k.join(", ")}`;
    };
    for (const [l, n] of refs) pose("Panneaux", "panneau_mur_m2", `Panneaux sandwich de mur ${ep} mm, ${fz(+l)} \xD7 ${fz(H * 100)} cm (${ou(+l)})`, n * (+l / 100) * H, `${n} panneau(x) entier(s) de ${fz(+l)} \xE0 commander (les bandes recoup\xE9es sortent des chutes)`);
    ...
    if (m.lisse) pose("Bois", "lambourde_ml", `Lambourde ${m.lisse.section_mm.join(" \xD7 ")} classe 4 (lisse haute \xE0 plat sur chaque mur, seuil de porte)`, m.lisse.nb_barres * m.lisse.longueur_stock_cm / 100, `${m.lisse.nb_barres} pi\xE8ce(s) de ${fz(m.lisse.longueur_stock_cm)} cm : une lisse par mur (${fr(rnd(perim, 2))} m)${m.lisse.seuil.length ? `, plus ${m.lisse.seuil.length} \xE9paisseurs de seuil` : ""}`);
    pose("Bois", "madrier_ml", `Madrier ${m.rehausse.section_mm.join(" \xD7 ")} classe 4 (rehausse)`, m.rehausse.nb_madriers * m.rehausse.longueur_stock_cm / 100, `${m.rehausse.nb_madriers} pi\xE8ce(s) de ${fz(m.rehausse.longueur_stock_cm)} cm`);
    ...
    pose("Fixations", "cheville_beton_u", "Chevilles ou goujons pour fixer les corni\xE8res de pied dans la dalle", 2 * (haut(perim / 0.5) + 2), "une tous les 50 cm, sur chacune des deux corni\xE8res");
    pose("\xC9tanch\xE9it\xE9", "bande_arase_ml", "Bande d'arase sous le pied des murs", perim, "p\xE9rim\xE8tre des murs");
    pose("\xC9tanch\xE9it\xE9", "butyle_ml", `Bande butyle (joints de panneaux, t\xEAte de mur sous la ${m.lisse ? "lisse" : "rehausse"})`, joints_murs * H + perim + (n_toit - 1) * m.portee_cm / 100, `${joints_murs} joints de mur \xD7 ${fz(H)} m + p\xE9rim\xE8tre + recouvrements de toit`);
    ...
    if (pl) {
      pose("Plancher isol\xE9", "film_pe_m2", "Film poly\xE9thyl\xE8ne sur la dalle", aire * 1.15, "surface int\xE9rieure, +15 % de recouvrements");
      pose("Plancher isol\xE9", "isolant_sol_m2", "Isolant XPS 60 mm, pos\xE9 \xE0 joints serr\xE9s sur le film", aire * 1.05, "surface int\xE9rieure, +5 %");
      pose("Plancher isol\xE9", "osb_m2", "Dalles OSB3 22 mm rainur\xE9es, coll\xE9es aux rainures", aire * 1.1, "surface int\xE9rieure, +10 % de chutes");
      pose("Plancher isol\xE9", "colle_rainure_u", "Colle \xE0 bois D3 pour les rainures", haut(aire / 10), "un flacon de 750 g pour 10 m\xB2 de dalles");
    ...
    pose("Ventilation", "aerateur_u", "Grilles ou entr\xE9es d'air murales, avec moustiquaire", 2, "une basse, une haute, sur deux murs oppos\xE9s");
  ...
  function guide_montage(p, v, m) {
    ...
    const seuil_porte = po ? +((m.faces[po.cote].ouvertures.find((o) => o.type === "porte") || {}).seuil_cm || 0) : 0;
    ...
    const etapes = [
      ...
      {
        titre: "Poser les corni\xE8res de pied",
        but: "Deux corni\xE8res alu tiennent le pied de chaque panneau, dehors et dedans : un rail en U fait sur place, qui se coupe \xE0 n'importe quel angle.",
        outils: ["perforateur", "visseuse", "scie \xE0 onglet lame alu", "niveau"],
        faire: [`D\xE9rouler la bande d'arase sur le trac\xE9 (${fr(rnd(perim, 2))} m), poser la **corni\xE8re ext\xE9rieure** dessus, aile debout dehors, **nu ext\xE9rieur sur le trait**.`, "Couper les corni\xE8res d'onglet \xE0 chaque angle (la moiti\xE9 de l'angle du mur), cheviller tous les 50 cm, et \xE0 10 cm de chaque angle.", po ? `Interrompre les corni\xE8res sur la largeur de la porte (${fz(po.largeur_cm + 2 * po.chambranle_cm)} cm, face ${face_porte.cle}).` : "", "Cordon de mastic continu entre la corni\xE8re et la dalle, c\xF4t\xE9 ext\xE9rieur.", `Les **corni\xE8res int\xE9rieures** se posent avec les murs : chacune serr\xE9e contre le pied des panneaux, donc \xE0 ${fz(ep)} cm de l'ext\xE9rieure sans avoir \xE0 mesurer.`].filter(Boolean),
        controler: ["Corni\xE8res de niveau : caler si la dalle a plus de 5 mm de faux niveau sur un mur.", "Angles conformes au trac\xE9 avant de cheviller le dernier mur."]
      },
      {
        ...
        faire: [
          ...
          ...m.lisse ? [`Lisse haute : ${m.lisse.pieces.map((x) => `${x.id} (mur ${x.face}, ${fr(x.L)} cm)`).join(", ")}, coup\xE9es d'onglet \xE0 la moiti\xE9 de chaque angle${m.lisse.seuil.length ? ` ; seuil : ${m.lisse.seuil.length} pi\xE8ces de ${fr(m.lisse.seuil[0].L)} cm` : ""}.`] : [],
      ...
      },
      {
        ...
        faire: [`Assembler ${F.G ? liste2(F.G) : ""} \xE0 plat, butyle dans chaque joint, et visser dessus ${m.lisse ? "leur lisse puis leur pi\xE8ce de rehausse" : "leur pi\xE8ce de rehausse"}.`, F.G && bande(F.G) ? `Placer la bande de ${fz(bande(F.G).largeur_cm)} cm c\xF4t\xE9 fa\xE7ade, la seule extr\xE9mit\xE9 qu'on atteindra ensuite.` : "", "Lever le mur \xE0 deux, le poser contre la corni\xE8re ext\xE9rieure, le tenir par deux \xE9tais viss\xE9s dans la rehausse.", "Poser la corni\xE8re int\xE9rieure contre le pied, la cheviller et la visser dans la t\xF4le : dehors, la corni\xE8re d\xE9j\xE0 pos\xE9e tient le pied, aucune vis n'est \xE0 faire c\xF4t\xE9 grillage."].filter(Boolean),
      ...
      },
      {
        ...
        faire: [
          ...
          "Butyle dans chaque embo\xEEtement, panneau serr\xE9 contre le pr\xE9c\xE9dent, pied pris entre les deux corni\xE8res et viss\xE9 dans chacune.",
          `\xC9tayer chaque mur tant que la ${m.lisse ? "lisse" : "rehausse"} n'est pas pos\xE9e : avant elle, rien ne tient les t\xEAtes.`
        ],
      ...
      {
        titre: m.lisse ? "Poser la lisse haute et la rehausse" : "Poser la rehausse bois",
        but: m.lisse ? "La lisse tient la t\xEAte de chaque mur, fond compris, et porte le bas du toit ; la rehausse pos\xE9e dessus donne la pente." : "Elle donne la pente au toit et sert de lisse haute : c'est elle qui tient les murs entre eux.",
        outils: ["visseuse", "serre-joints"],
        faire: m.lisse ? [`Poser la lisse \xE0 plat sur chaque mur (${m.lisse.pieces.map((x) => x.id).join(", ")}), sur un cordon de butyle en t\xEAte de panneaux, et la visser dans la t\xF4le des deux faces tous les 40 cm.`, "Lier les lisses entre elles \xE0 chaque angle par deux longues vis en biais.", `Poser ${m.rehausse.pieces.map((r) => `${r.id} sur ${r.face}`).join(", ")} sur la lisse, viss\xE9es dedans par-dessus tous les 40 cm.`] : [`Poser ${m.rehausse.pieces.map((r) => `${r.id} sur ${r.face}`).join(", ")}, sur un cordon de butyle en t\xEAte de panneaux.`, "Visser la rehausse dans la t\xF4le des deux faces de chaque panneau, tous les 40 cm.", "Assembler les pi\xE8ces entre elles aux angles par deux longues vis en biais."],
      ...
      }] : [],
      {
        ...
        faire: [
          ...
          `Visser dans la rehausse${m.lisse ? " (au fond : dans la lisse)" : ""}${t.panne_intermediaire ? " et dans la panne" : ""} par le sommet des nervures, vis longues \xE0 rondelle, quatre par panneau et par appui. Serrer jusqu'\xE0 \xE9craser la rondelle, pas plus.`,
      ...
      },
      {
        ...
        faire: [`${fr(G.longueur_cm)} cm de goutti\xE8re en ${G.troncons.length} tron\xE7on(s) : ${G.troncons.map((x) => `${x.face} ${fr(x.longueur_cm)} cm`).join(" + ")}.`, "Crochets tous les 50 cm, pente de 5 mm par m\xE8tre vers la descente.", t.descente === "droite" && m.sens !== "droite" ? "Descente au bout droit de la goutti\xE8re, puis un tuyau au sol le long du mur droit jusqu'au jardin : rien ne doit s'\xE9couler au pied du mur de propri\xE9t\xE9." : t.descente === "gauche" && m.sens !== "droite" ? "Descente au bout gauche de la goutti\xE8re, au coin arri\xE8re gauche, puis un tuyau au sol le long de la limite jusqu'au jardin." : "Descente au point bas, \xE9vacu\xE9e loin de la dalle."],
      ...
      ...p.amenagement && p.amenagement.plancher && p.amenagement.plancher.actif ? [{
        ...
        faire: ["Film poly\xE9thyl\xE8ne sur la dalle, remont\xE9 de 10 cm le long des murs, l\xE9s recouverts de 20 cm.", "Isolant XPS de 60 mm pos\xE9 \xE0 joints serr\xE9s sur tout le sol, sans vis ni colle : c'est un plancher flottant.", "Dalles OSB de 22 mm pos\xE9es dessus, joints d\xE9cal\xE9s, colle dans chaque rainure, 8 mm de jeu contre les murs ; rev\xEAtement de sol ensuite."],
        controler: ["Avant le film : la dalle est plane \xE0 5 mm pr\xE8s sous une r\xE8gle de 2 m, sinon ragr\xE9er les creux.", `Hauteur sous plafond apr\xE8s plancher : ${fr(m.sous_plafond_m.haut)} m au plus haut, ${fr(m.sous_plafond_m.bas)} m au plus bas.`]
      }] : [],
      {
  ...
  function geometry(p) {
    ...
    const murs = noms.map((nom, i) => ({ nom, i })).filter(({ nom }) => est_mur(nom)).map(({ nom, i }) => {
      ...
      return { cote: nom, type: grillages.includes(nom) ? "grillage" : palissades.includes(nom) ? "palissade" : "mur", de: [rnd2(a[0], 1), rnd2(a[1], 1)], a: [rnd2(b[0], 1), rnd2(b[1], 1)] };
    });
    let zone_utile = null;
    ...
    return {
      dalle: {
        ...
        polygone: local.map(([x, y]) => [rnd2(x, 1), rnd2(y, 1)]),
  ...
  function variantes(p, g) {
    ...
    const fond = g.dalle.murs.filter((w) => w.cote.startsWith("arriere")).map((w) => ({ cote: w.cote, a: w.de, b: w.a }));
    const pied = (q, a, b) => {
    ...
    const dalle_abs = g.dalle.polygone;
    ...
    for (const v of out) {
      ...
      v.lits_muraux = (disp.lits_muraux || []).map((lm) => {
        const k = v.noms_cotes.indexOf(lm.contre);
        if (k < 0) return null;
...
`;
    ...
    if (m && v && d.gaine) {
      const gp = [d.gaine.x_cm, d.gaine.y_cm], c = P(gp), r = Math.max(d.gaine.diametre_cm / 2 * scale, 3.5);
      const dans = (z, poly_) => {
        let o = false;
        for (let i = 0, j = poly_.length - 1; i < poly_.length; j = i++) {
          const a = poly_[i], b = poly_[j];
          if (a[1] > z[1] !== b[1] > z[1] && z[0] < (b[0] - a[0]) * (z[1] - a[1]) / (b[1] - a[1]) + a[0]) o = !o;
        }
        return o;
      };
      const sous = dans(gp, v.polygone) && !dans(gp, m.interieur), face2 = sous ? m.faces.reduce((best, f) => {
        const l = Math.hypot(f.a[0] - f.de[0], f.a[1] - f.de[1]) || 1, dist = Math.abs((f.a[0] - f.de[0]) * (f.de[1] - gp[1]) - (f.de[0] - gp[0]) * (f.a[1] - f.de[1])) / l;
        return !best || dist < best.d ? { f, d: dist } : best;
      }, null).f : null;
      const ou = sous ? `sous le mur ${face2.cle}` : dans(gp, m.interieur) ? "dans l'abri" : "hors de l'abri";
...
`;
      svg += text(c[0], yl, `gaine \xE9lectrique \xD8${fz2(d.gaine.diametre_cm)} \xB7 ${fz2(d.gaine.x_cm)} depuis la gauche, ${fz2(d.gaine.y_cm)} depuis l'avant \xB7 ${ou}`, "middle", "#c0392b", 10, "bold");
    }
  ...
  function modele_trapeze(p, v) {
    ...
    const h = (z) => Hl + c * (1 - (droite ? (z[0] - x0) / Wd : (z[1] - y0) / D));
    ...
    const pieces = faces.filter((f) => Math.max(f.hauteur_debut_cm, f.hauteur_fin_cm) > Hl + 0.05).map((f, k) => ({ id: `R${k + 1}`, face: f.cle, L: f.longueur_cm, h0: rnd2(f.hauteur_debut_cm - Hl, 1), h1: rnd2(f.hauteur_fin_cm - Hl, 1) }));
    ...
    for (const f of faces) for (const pn of f.panneaux) {
      ...
      pn.decoupes = f.ouvertures.filter((o) => o.debut_cm - (o.chambranle_cm || 0) < pn.debut_cm + pn.largeur_cm && o.debut_cm + o.largeur_cm + (o.chambranle_cm || 0) > pn.debut_cm).map((o) => `${o.type === "porte" ? "porte" : "fen\xEAtre"} ${fz2(o.largeur_cm + 2 * (o.chambranle_cm || 0))} \xD7 ${fz2(o.hauteur_cm + (o.chambranle_cm || 0) + (o.seuil_cm || 0))}`);
    }
    ...
    return {
      ...
      toit: {
        ...
        plan: { sens: droite ? "droite" : "arriere", origine_cm: rnd2(droite ? x0 : y0, 1), course_cm: rnd2(course, 1), haut_cm: Hl + c, bas_cm: Hl },
...
`;
  ...
  function resume_svg(g, v, m) {
    const d = g.dalle, dalle = d.polygone, q = v.polygone;
...
`;
  ...
  function entete_sol(p, v) {
    const mural_e = (v.lits_muraux || []).find((lm) => lm.tient);
    const lignes = [`murs ${fz2(+p.panneau.epaisseur_mm / 10)} cm \xB7 porte ${fz2(v.porte.largeur_cm)} ouvrant dehors \xB7 fen\xEAtres en bleu${v.sol_libre_m2 != null ? ` \xB7 sol libre ${v.sol_libre_m2} m\xB2` : ""}${mural_e ? " \xB7 violet = lit \xE0 demeure" : v.lit_pliant && v.lit_pliant.tient ? " \xB7 violet pointill\xE9 = lit d\xE9pli\xE9" : ""}`];
  ...
  function modele_sol_svg(p, v, m, sans_entete = false) {
    ...
    const mural = (v.lits_muraux || []).find((lm) => lm.tient);
    const cotes_rect = (z) => {
      const a = Math.hypot(z[1][0] - z[0][0], z[1][1] - z[0][1]), b = Math.hypot(z[2][0] - z[1][0], z[2][1] - z[1][1]);
      return [rnd2(Math.min(a, b), 1), rnd2(Math.max(a, b), 1)];
    };
    const bureaux = mural ? mural.bureaux.map((b) => ({ ...b, profondeur_cm: cotes_rect(b.polygone)[0], longueur_cm: cotes_rect(b.polygone)[1] })) : v.bureaux || [];
    const sieges = mural ? mural.sieges.map((s) => ({ ...s, tient: true, profondeur_cm: cotes_rect(s.polygone)[0], largeur_cm: cotes_rect(s.polygone)[1] })) : v.sieges || [];
    if (mural) {
      svg += poly(mural.polygone.map(P), "#e7dcf5", "#6a3d9a", 1.8);
      const [w_lit, l_lit] = cotes_rect(mural.polygone);
      const cm = P(mural.polygone.reduce((s, z) => [s[0] + z[0] / 4, s[1] + z[1] / 4], [0, 0]));
      svg += text(cm[0], cm[1] - 3, `lit ${fz2(w_lit)} \xD7 ${fz2(l_lit)}`, "middle", "#6a3d9a", 11, "bold");
      ...
      const cotesq = mural.polygone.map((a, i) => ({ a, b: mural.polygone[(i + 1) % 4] }));
      const courts = cotesq.filter((c) => Math.hypot(c.b[0] - c.a[0], c.b[1] - c.a[1]) < (w_lit + l_lit) / 2).sort((c1, c2) => sens * (c1.a[k] + c1.b[k] - (c2.a[k] + c2.b[k])));
      ...
      if (tete) {
        ...
        const dedans = [tete.a, tete.b, [tete.b[0] - nx * 25 * dir, tete.b[1] - ny * 25 * dir], [tete.a[0] - nx * 25 * dir, tete.a[1] - ny * 25 * dir]];
        const ok_dedans = poly_area(clip_convex(dedans, mural.polygone)) > 100;
        const bande = ok_dedans ? dedans : [tete.a, tete.b, [tete.b[0] + nx * 25, tete.b[1] + ny * 25], [tete.a[0] + nx * 25, tete.a[1] + ny * 25]];
...
`;
  ...
  function modele_facade_svg(m, f, sans_entete = false, largeur_commune = 0) {
    ...
    const E = f.lisse_cm || 0, Hr = Hm + E;
    if (E > 0) svg += poly([P(0, Hm), P(L, Hm), P(L, Hr), P(0, Hr)], "#c9a26b", "#8a5a2b", 1.2);
    if (Math.max(h0, h1) > Hr + 0.05) {
      const pc = m.rehausse.pieces.find((x) => x.face === f.cle);
      svg += poly([P(0, Hr), P(L, Hr), P(L, h1), P(0, h0)], "#d9b98a", "#8a5a2b", 1.5);
      const c = P(L / 2, Math.max(h0, h1) - Hr > 16 ? Hr + 6 : Hr + 2);
      svg += text(c[0], c[1] - 2, `${pc ? pc.id + " \xB7 " : ""}rehausse ${fz2(rnd2(h0 - Hr, 1))} \u2192 ${fz2(rnd2(h1 - Hr, 1))}${E > 0 ? ` \xB7 lisse ${fz2(E)}` : ""}`, "middle", "#5a3a1a", 10, "bold");
    } else if (E > 0) {
      const c = P(L / 2, Hr + 3);
      svg += text(c[0], c[1], `lisse ${fz2(E)}`, "middle", "#5a3a1a", 10, "bold");
    }
    for (const o of f.ouvertures) {
      if (o.chambranle_cm) {
...
`;
      ...
      if (o.seuil_cm) svg += poly([P(o.debut_cm, 0), P(o.debut_cm + o.largeur_cm, 0), P(o.debut_cm + o.largeur_cm, o.seuil_cm), P(o.debut_cm, o.seuil_cm)], "#c9a26b", "#8a5a2b", 1.2);
...
`;
      ...
      svg += text(c[0], c[1] + 12, `${fz2(o.largeur_cm)} \xD7 ${fz2(o.hauteur_cm)}${o.allege_cm ? ` \xB7 ${o.type === "porte" ? "seuil" : "all\xE8ge"} ${fz2(o.allege_cm)}` : ""}`, "middle", col, 10);
  ...
  function injecteur(v, m) {
    ...
    const valeurs = {
      ...
      panneaux_mur: String(m.panneaux_mur_a_commander),
      module_cm: fz2(m.faces[0].module_cm),
      bureau_cm: v.bureaux && v.bureaux[0] ? fz2(v.bureaux[0].longueur_cm) : "?"
    };
  ...
  var import_OrbitControls = __toESM(require_OrbitControls(), 1);
  var import_RoomEnvironment = __toESM(require_RoomEnvironment(), 1);
  ...
  var COUL = { mur: 14672348, joint: 4871262, bois: 12752218, toit: 10134443, nervure: 8358290, dalle: 13223355, propriete: 11049606, sol: 7305858, bureau: 14268810, siege: 4938346, lit: 9333688, porte: 9279391, cadre: 11105343, verre: 10474470, metal: 11186873, personne: 3829413, grillage: 5204799, palissade: 10121800, poteau: 7294766 };
  ...
  function peuple_abri(abri, data, visible_demande = {}) {
    ...
    if (data.gaine) {
      ...
      const tube = new THREE.Mesh(new THREE.CylinderGeometry(dia / 2, dia / 2, h, 16), mat(12597547, { roughness: 0.6 }));
      tube.position.copy(W(data.gaine.x_cm, data.gaine.y_cm, h * 50));
      g.add(ombre(tube));
    }
      ...
      for (const o of f.ouvertures) {
        if (o.type === "porte") {
          ...
          const battant = (angle) => {
            ...
            g.position.set(s1 / 100, seuil / 100, -0.01);
            g.rotation.y = angle;
            return g;
          };
  ...
  function createAbriViewer(container, data0) {
    ...
    scene.environment = pmrem.fromScene(new import_RoomEnvironment.RoomEnvironment(), 0.04).texture;
